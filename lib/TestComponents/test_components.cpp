...
void calibrateThreshold() {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        Serial.print(i + 1);
        Serial.print(": "));
        Serial.println(F(maxLineValues[i]);
    }
    ...
    for (int i = 0; i < 6; i++) {
        ...
        Serial.print(i + 1);
        Serial.print(": "));
        Serial.println(F(minBackgroundValues[i]);
    }
    ...
    Serial.println(F(avgLine);
    Serial.print("Valor médio no FUNDO: "));
    Serial.println(F(avgBackground);
    Serial.print("THRESHOLD RECOMENDADO: "));
    Serial.println(F(recommendedThreshold);
